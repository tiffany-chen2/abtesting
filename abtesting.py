...
def get_2_sample_df(a, b):
    ...
    denoma = (((get_stdev(a)**2)/len(a))**2)/(len(a)-1)
    denomb = (((get_stdev(b)**2)/len(b))**2)/(len(b)-1)
    sum = denoma + denomb
    se = se**4
    df = round(se/sum)
    return df
...
def row_sum(observed_grid, ele_row):
    sum = 0
    for n in range(0, len(observed_grid[ele_row])):
        sum+=observed_grid[ele_row][n]
    return sum

def col_sum(observed_grid, ele_col):
    sum = 0
    for n in range(0, len(observed_grid)):
        sum+=observed_grid[n][ele_col]
    return sum

def total_sum(observed_grid):
    sum = 0
    for i in range(0, len(observed_grid)):
        for j in range(0, len(observed_grid[i])):
            sum+=observed_grid[i][j]
    return sum

def calculate_expected(row_sum, col_sum, tot_sum):
    exp = (row_sum*col_sum)/tot_sum
    return exp
...
def get_expected_grid(observed_grid):
    ...
    ** DO NOT modify the parameter, observed_grid. **
    ...
    '''
    t = total_sum(observed_grid)
    rows, cols = (len(observed_grid), len(observed_grid[0]))
    arr = [[0 for i in range(cols)] for j in range(rows)] 
    for i in range(0, len(observed_grid)):
        for j in range(0, len(observed_grid[i])):
            r = row_sum(observed_grid, i)
            c = col_sum(observed_grid, j)
            arr[i][j] = calculate_expected(r, c, t)
    return arr
...
def chi2_value(observed_grid):
    ...
    '''
    chi = 0
    expgrid = get_expected_grid(observed_grid)
    for i in range(0, len(observed_grid)):
        for j in range(0, len(observed_grid[i])):
            chi += ((observed_grid[i][j]-expgrid[i][j])**2)/expgrid[i][j]
    return chi
